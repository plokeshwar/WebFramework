...
public class TS_26_Verify_ContentEditorPage_CommonAcrossAllPagesLink {
	
	String className=this.getClass().getName().substring(27);
	
	WebDriver driver = null;
	String mainDriver=null, subDriver=null;
	...
	public void TC_04_Validate_PopUpBody_ContentRegion() {
		mainDriver=co.controlOfMainDriver(driver);
		ArrayList<String> stringList=cm.linkFinder(driver, contentEditorPage.listPageTable);
		for(int i=0; i<stringList.size();i++)
		{
			WebElement element= driver.findElement(By.xpath("//td[@class='listPage']//a[contains(.,'"+stringList.get(i)+"')]")) ;
			...
			subDriver=co.controlOfSubDriver(driver);
			co.driverSwitch(driver, subDriver, "Passing driver control to the subDriver.");
			
			co.click(contentEditorPage.contentRegionNamePopUpButton, "Clicking on Save Changes Button");
			co.alertHandler(driver).accept();
			co.click(contentEditorPage.popUpCloseCommonAAP, "Closing Pop Up");
			co.driverSwitch(driver, mainDriver, "Passing the driver control to Main Driver.");
	...
	public void TC_08_Validate_ContentRegionPopUp_SaveChanges_Alert_OKFunction() {
		...
		mainDriver=co.controlOfMainDriver(driver);
		String text="Pravin University Testing Number "+cm.randomNumberGenerator();
		...
		{
			contentEditorPage=new ContentEditorPage().contentEditorInit(driver);
			element= driver.findElement(By.xpath("//td[@class='listPage']//a[contains(.,'"+stringList.get(i)+"')]")) ;
			co.click(element, "Clicking on "+stringList.get(i));
			driver.switchTo().frame(contentEditorPage.frameID);
			co.click(contentEditorPage.contentRegionNamePopUpBody, "Clicking on the Body");
			co.type (contentEditorPage.contentRegionNamePopUpBody, text, "Typing inside the body");
			subDriver=co.controlOfSubDriver(driver);
			co.driverSwitch(driver, subDriver, "Driver Control Passing to sub Driver.");
			...
			co.driverSwitch(driver, mainDriver, "Driver Control Passing to Main Driver.");
			
			contentEditorPage=new ContentEditorPage().contentEditorInit(driver);
			co.click(contentEditorPage.popUpCloseCommonAAP, "Closing Pop Up");
			Reporter.log(cm.ReporterTextBold("Expected : "+text));
			Reporter.log(cm.ReporterTextBold("Actual : "+actualText));
			Assert.assertEquals(actualText, text, "Tested text is not Saved For "+stringList.get(i));
		}		
	}
	//===========================================================================================================================
	@BeforeMethod
